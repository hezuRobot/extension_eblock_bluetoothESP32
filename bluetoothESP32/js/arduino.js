...
(function(ext) {
  ...
  ext.avaibleBT = function(nextID){
		var deviceId = 64;              // en function solo pido nextID que es para enviar el getPackage
		getPackage(nextID,deviceId,0); //envio pin=0 para indicar que pida BT.avaible()
	}
  ext.readBT = function(nextID){
		var deviceId = 64;              // en function solo pido nextID que es para enviar el getPackage
		getPackage(nextID,deviceId,1); //envio pin=1 para indicar que pida BT.read()
	}
  ext.writeBT = function(texto){
		var deviceId = 64;
    //texto = texto.toString().charCodeAt(0); // este va perfecto con codigo ascii
            //texto = texto.toString().charCodeAt(1); //he probado esto para ver si guarda la cadena y manda el segundo elemento
                                            // y lo hace perfectamente con codigo ascii como el anterior
                                            //este es el que funciona con numeros
    //runPackage(deviceId,texto);             // junto con el anterior envio el ascii



    texto = texto.toString();
    runPackage(deviceId,texto.length,string2array(texto));
    //runPackage(deviceId,texto);
    //texto = texto.charCodeAt(0);  //si no convierto primero a string no va
    //cadena = texto.toString().charCodeAt(0);
    //runPackage(deviceId,cadena);
    //runPackage(deviceId,texto.length,texto);  //envia solo ^A
    //runPackage(deviceId,texto.length,_util.string2array(texto)); //no envia nada




      //device.send([0xff, 0x55, 4, 0, 2, 64, texto);
      //device.send([0xff, 0x55, 4, 0, 2, 64, parseInt(texto, 16));


		//sendPackage(deviceId,texto,2);        //deviceId 64 no se si sera necesario
    //vamos a crear un nuevo modelo de paquete con identificador 6
    //1,2,4 y 5 ya son para get, run, reset y start
    //sendPackage(deviceId,texto, 6);
	}
/*
...
*/
	...
	function sendPackage(argList, type){
		...
		device.send(bytes); //este es el que funciona pero solo envia numeros no caracteres
    //device.writeRaw(bytes); //esto es una prueba y no funciona
    //device.sendRaw(bytes); //otra prueba fallida
	}
